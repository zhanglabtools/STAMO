...
def integration_consistency(
        model: STAMOModel, adatas: Mapping[str, AnnData],
    ...
    **kwargs
    ...
    for k, adata in adatas.items():
        adata.obsm["X_STAMO"] = model.encode_data(k, adata)
    ...
    for n_meta in (10, 20, 50, 100, 200):
        ...
        corr = metacell_corr(
            *adatas.values(), skeleton=graph,
            use_rep="X_STAMO", n_meta=n_meta, **kwargs
        )
